...
public class WebVisitableTester {
	...
	public WebVisitableTester(String rootLink, String rootLinkTitle, int maxRequests, int minRequests, int maxTestDepth,
	        boolean testExternalLink) {
		super();
	...
	public void print(LinkStatsEntry linkStatsEntry, int deepth) {
		if (null != linkStatsEntry) {
			for (int i = 0; i < deepth; ++i)
				System.out.print(" ");
			String link = linkStatsEntry.getLink().length() > 32 ? linkStatsEntry.getLink().substring(0, 29) + "..." : linkStatsEntry
			        .getLink();
			System.out.println(String.format("%s\t\t%s\t%d\t%d\t%d\t%d\t%d\t", link, linkStatsEntry.getTitle(),
			        linkStatsEntry.getResponseCode(), linkStatsEntry.getRequestedCount(), linkStatsEntry.getMaxResponseTime(),
			        linkStatsEntry.getMinResponseTime(), linkStatsEntry.getAvarageResponseTime()));
		}
	}
